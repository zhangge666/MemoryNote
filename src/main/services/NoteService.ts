...
export class NoteService {
  ...
  async updateNote(options: UpdateNoteOptions): Promise<Note | null> {
    ...
    if (options.content !== undefined) {
      // 读取旧内容用于 diff 比较
      oldContent = await this.fs.readNote(note.filePath);
      ...
      if (reviewConfig.autoGenerateCards) {
        // 自动生成复习卡片
        try {
          const reviewService = getReviewService();
          await reviewService.generateCardsFromDiff(options.id, oldContent, options.content);
          console.log(`📝 Auto-generated review cards for note ${options.id}`);
        } catch (error) {
          console.error('Failed to auto-generate review cards:', error);
        }
      }
    }
