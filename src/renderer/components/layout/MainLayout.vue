<template>
  <div class="main-layout flex flex-col h-screen w-screen overflow-hidden bg-background text-text">
    ...
    <div class="flex flex-1 overflow-hidden">
      ...
      <ResizableSidebar
        side="left"
        :visible="sidebarStore.leftVisible"
        :width="sidebarStore.leftWidth"
        @resize-start="sidebarStore.startResize('left')"
        @resize="sidebarStore.setLeftWidth"
        @resize-end="sidebarStore.stopResize"
      >
        <!-- 根据导航状态渲染不同内容 -->
        <FileTree
          v-if="navigationStore.activeView === 'notes' || navigationStore.activeView === 'journal'"
          ref="fileTreeRef"
          @select-note="handleSelectNote"
        />
        <ReviewSidebar
          v-else-if="navigationStore.activeView === 'review'"
          ref="reviewSidebarRef"
        />
        <SearchSidebar
          v-else-if="navigationStore.activeView === 'search'"
          ref="searchSidebarRef"
        />
        <PluginSidebar
          v-else-if="navigationStore.activeView.startsWith('plugin:')"
          :view-id="navigationStore.activeView"
        />
      ...
      <ResizableSidebar
        side="right"
        :visible="sidebarStore.rightVisible"
        :width="sidebarStore.rightWidth"
        @resize-start="sidebarStore.startResize('right')"
...
import ReviewSidebar from '@renderer/components/review/ReviewSidebar.vue';
...
import ReviewView from '@renderer/views/ReviewView.vue';
...
const reviewSidebarRef = ref<any>();
const searchSidebarRef = ref<any>();
...
async function loadStatusBarData() {
  try {
    ...
    // 获取复习统计
    try {
      const reviewResponse = await window.ipc.review.getStats();
      if (reviewResponse.success && reviewResponse.data) {
        reviewCount.value = reviewResponse.data.dueCards;
      }
    } catch (e) {
      reviewCount.value = 0;
    }
