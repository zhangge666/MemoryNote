<template>
  <div class="navbar flex flex-col h-full flex-shrink-0">
    ...
    <div class="flex-1 flex flex-col items-center py-3 gap-2">
      <NavButton
        v-for="item in topItemsWithPlugins"
        :key="item.id"
        :icon="item.icon"
        :title="item.pluginView ? item.pluginView.title : t(`navbar.${item.id}`)"
...
const selectView = (viewId: string) => {
  ...
  // 仅底部按钮需要打开工作区标签页
  const sidebarOnlyViews = ['notes', 'journal', 'review', 'search'];
  
  // 插件视图也是侧边栏视图
  if (viewId.startsWith('plugin:')) {
    return;
  }
  if (sidebarOnlyViews.includes(viewId)) {
    // 这些视图只切换侧边栏内容，不打开标签页
    return;
  }
